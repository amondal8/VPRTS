...
def valuesR1andR2Added(rwCount1, rwCount2):  # configuration=3
    userStory_PointR1 = int(worksheet1.cell(row=rwCount1, column=3).value)
    userStory_PointR2 = int(worksheet2.cell(row=rwCount2, column=3).value)
    if worksheet1.cell(row=rwCount1, column=4).value is None:
        userStory_Pointadd = 0
    else:
        usp = worksheet1.cell(row=rwCount1, column=4).value
        userStory_Pointadd = int(usp)

    userStory_Pointadd = userStory_PointR1 + userStory_PointR2 + userStory_Pointadd
    cell = worksheet1.cell(row=rwCount1, column=4)  # writing the new value on R1 sheet: repeat of R1 is getting calculated
...
def valuesR1andR2Added2(rwCount1, rwCount2):  # configuration=3
    userStory_PointR1 = int(worksheet1.cell(row=rwCount1, column=3).value)
    userStory_PointR2 = int(worksheet2.cell(row=rwCount2, column=3).value)
    if worksheet2.cell(row=rwCount2, column=4).value is None:
        userStory_Pointadd = 0
    else:
        usp = worksheet2.cell(row=rwCount2, column=4).value
        userStory_Pointadd = int(usp)

    userStory_Pointadd = userStory_PointR1 + userStory_PointR2 + userStory_Pointadd
    cell = worksheet2.cell(row=rwCount2, column=4)  # writing the new value on R2 sheet: repeat of R2 is getting calculated
    writeto_excel(cell, userStory_Pointadd)
