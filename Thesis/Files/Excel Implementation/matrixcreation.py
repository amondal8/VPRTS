...
def generate_random_adjacency_matrix(n, m, p):
    # Create an empty adjacency matrix of size n x n
    adjacency_matrix = np.zeros((n, m))

    # Fill the adjacency matrix with random connections
    for i in range(n):
        for j in range(m):
            if np.random.rand() < p:
                # Set the connection between nodes i and j
...
# m = 500  # Number of cols
# p = .1  # Probability of connection

# adj_matrix = generate_random_adjacency_matrix(n, m, p)
# print(adj_matrix)
