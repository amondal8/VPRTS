...
def create_tcdict_exectime(tc_list, ds_id):
    keyval_dict = {}
    for i in tc_list:
        query = f"""select tc_executiontime, tc_setuptime, tc_teardowntime, tc_additionalres
                    from tc_datasettable 
                    where tc_id = '{i}' and ds_id = '{ds_id}'; """
        tcexectime_res = ut_ds.running_searchqury(query)
        # print(tcexectime_res[0][0])
        exectime = int(tcexectime_res[0][0]) + int(tcexectime_res[0][1]) + int(tcexectime_res[0][2]) + int(tcexectime_res[0][3])
        keyval_dict[i] = exectime
    return keyval_dict
